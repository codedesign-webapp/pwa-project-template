<template>
  <v-container>
    <v-row>
      ...
      <v-col offset="3" cols="6" class="text-center mt-5">
        <!-- 구글 계정 로그인 버튼(redirect) 표시 및 처리 -->
        <v-btn @click="fnDoGoogleLogin_Redirect()" block outlined color="blue" large dark>
        <!-- 머티리얼디자인아이콘 사용 시 아이콘 이름에 'mdi-' 붙임 -->
...
<script>
  export default {
    methods: {
      ...
      fnDoGoogleLogin_Redirect() { // 스토어에 구글계정 로그인(redirect) 처리 요청
        this.$store.dispatch("fnDoGoogleLogin_Redirect")
      }
    },
    computed: {
      // 시간지연상태 스토어에서 읽어서 반환
